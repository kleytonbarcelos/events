...
<body>
			...
			$('body').on('submit', '#formLogin', function(event)
			{
				...
				$.ajax(
				{
					...
					success: function(data)
					{
						...
						if( data.status == 1 )
						{
							$form.replaceWith('<div class="alert alert-success">'+data.msg+'</div><div class="font-center"><a href="'+base_url+'login"><small><i class="fa fa-angle-double-left"></i> ir para login</small></a></div>');
							setTimeout(function()
							{
	...
	<div class="container">
		...
		<div class="card card-container">
			...
			<hr>
			<div class="msg"></div>
			<?=form_open('login/salvarnovasenha', array('id'=>'formLogin', 'class'=>'form-signin'))?>
				<input type="hidden" name="key" value="<?=$key?>">
				<span id="reauth-email" class="reauth-email"></span>
				<input type="text" id="txtEmail" name="txtEmail" readonly="readonly" class="login_box" value="<?=$email?>">
				<input type="password" id="txtSenha" name="txtSenha" class="login_box" placeholder="Senha">
				<input type="password" id="txtRepetirSenha" name="txtRepetirSenha" class="login_box" placeholder="Repetir Senha">
